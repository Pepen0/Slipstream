...
\node[box, below=30mm of io, minimum width=68mm, align=center] (bus) {\textbf{Realtime Field Bus}\\CAN-FD (multi-drop) \textit{or} Multi-USB Hub\\\textit{Expansion-ready: +Yaw/+Heave/+Surge}};

\node[box, minimum width=58mm, align=left] (mcu_wheel) at ($(bus.south)+(-50mm,-16mm)$) {\textbf{Wheel MCU}\\1 kHz FFB haptics\\Inputs: encoder/torque\\Outputs: torque setpoint \& status};
\node[box, minimum width=58mm, align=left] (mcu_pedals) at ($(bus.south)+(0mm,-16mm)$) {\textbf{Pedals MCU}\\1 kHz sampling (loadcells/halls)\\Filters, debouncing\\Outputs: pedal positions};
\node[box, minimum width=58mm, align=left] (mcu_motion) at ($(bus.south)+(50mm,-16mm)$) {\textbf{Motion MCU (2-DOF)}\\PID @1 kHz\\Targets: \textbf{Pitch $\pm$15°}, \textbf{Roll $\pm$12°}\\Hall feedback \& limits};

\node[box, below=6mm of mcu_wheel, minimum width=58mm, align=left] (act_wheel) {\textbf{Wheel Actuator}\\Direct-drive motor + driver (less than 5 Nm, 360°)};
\node[box, below=6mm of mcu_pedals, minimum width=58mm, align=left] (act_pedals) {\textbf{Pedal Assemblies}\\Throttle / Brake / Clutch (std. mounts)};
\node[box, below=6mm of mcu_motion, minimum width=58mm, align=left] (act_motion) {\textbf{2-DOF Platform}\\Two 24 V DC motors, worm gears\\U-joint pivot; mechanical stops (±17°)};

\node[box, below=12mm of act_pedals, minimum width=78mm, align=left] (power) {\textbf{Power \& Safety}\\24 V, 42 A PSU; inline fuses;\\E-Stop relay cuts 24 V bus;\\120 VAC $\leq$ 1,000 W};
